...
describe('BitMath', () => {
  ...
  //     //   [...Array(10)].map((_, i) => bitMath.mostSignificantBit(BigNumber.from(2).pow(i)))
  //     // ));
  //     // console.log([...Array(10)].map((_, i) => i));
  //     expect(results).to.deep.eq([...Array(255)].map((_, i) => i))
  //   })
  //   it('uint256(-1)', async () => {
  //     expect(await bitMath.mostSignificantBit(BigNumber.from(2).pow(256).sub(1))).to.eq(255) // 运算结果还需要是正数
  //   })

  //   it('gas cost of smaller number', async () => {
  //     // console.log((BigNumber.from(3568))); // 数字转成 BigNumber
  //     // console.log((await bitMath.getGasCostOfMostSignificantBit(BigNumber.from(3568))).toString()); // 算出花费的 gas
  //     await snapshotGasCost(bitMath.getGasCostOfMostSignificantBit(BigNumber.from(3568)))
  //   })
  //   it('gas cost of max uint128', async () => {
  //     // console.log((await bitMath.getGasCostOfMostSignificantBit(BigNumber.from(2).pow(128).sub(1))).toString()); // 算出花费的 gas
  ...
  //     )
  //     expect(results).to.deep.eq([...Array(255)].map((_, i) => i))
  //     // console.log([...Array(255)].map((_, i) => i));
  //   })
  //   it('uint256(-1)', async () => {
